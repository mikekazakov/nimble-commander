...
vector<uint8_t> Hash::Final()
{
    switch (m_Mode) {
        case SHA1_160: {
            vector<uint8_t> r(CC_SHA1_DIGEST_LENGTH);
            CC_SHA1_Final( r.data(), (CC_SHA1_CTX*)m_Stuff );
            return r;
        }
        case SHA2_224: {
            vector<uint8_t> r(CC_SHA224_DIGEST_LENGTH);
            CC_SHA224_Final( r.data(), (CC_SHA256_CTX*)m_Stuff );
            return r;
        }
        case SHA2_256: {
            vector<uint8_t> r(CC_SHA256_DIGEST_LENGTH);
            CC_SHA256_Final( r.data(), (CC_SHA256_CTX*)m_Stuff );
            return r;
        }
        case SHA2_384: {
            vector<uint8_t> r(CC_SHA384_DIGEST_LENGTH);
            CC_SHA384_Final( r.data(), (CC_SHA512_CTX*)m_Stuff );
            return r;
        }
        case SHA2_512: {
            vector<uint8_t> r(CC_SHA512_DIGEST_LENGTH);
            CC_SHA512_Final( r.data(), (CC_SHA512_CTX*)m_Stuff );
            return r;
        }
        case MD2: {
            vector<uint8_t> r(CC_MD2_DIGEST_LENGTH);
            CC_MD2_Final( r.data(), (CC_MD2_CTX*)m_Stuff );
            return r;
        }
        case MD4: {
            vector<uint8_t> r(CC_MD4_DIGEST_LENGTH);
            CC_MD4_Final( r.data(), (CC_MD4_CTX*)m_Stuff );
            return r;
        }
        case MD5: {
            vector<uint8_t> r(CC_MD5_DIGEST_LENGTH);
            CC_MD5_Final( r.data(), (CC_MD5_CTX*)m_Stuff );
            return r;
        }
        case Adler32:
        case CRC32:
            return vector<uint8_t>{ m_Stuff[3], m_Stuff[2], m_Stuff[1], m_Stuff[0] };
        default: assert(0);
    }
    return vector<uint8_t>();
}

string Hash::Hex(const vector<uint8_t> &_d)
{
