...
static const auto g_SendingDelay = std::chrono::minutes(10);
...
static std::string GetStoredOrNewClientID()
{
...
// quite inefficient
static std::string EscapeString(const char *_original)
{
    static const auto acs = NSCharacterSet.URLQueryAllowedCharacterSet;
    return
        [[NSString stringWithUTF8String:_original] stringByAddingPercentEncodingWithAllowedCharacters:acs].UTF8String;
}

static std::string EscapeString(const std::string &_original)
{
...
static NSString *GetUserAgent()
{
    ...
    NSString *UA = [NSString stringWithFormat:@"GoogleAnalytics/3.0 (Macintosh; Intel %@ %@; %@-%@)",
                                              osInfo[@"ProductName"],
                                              [osInfo[@"ProductVersion"] stringByReplacingOccurrencesOfString:@"."
                                                                                                   withString:@"_"],
...
GoogleAnalytics::GoogleAnalytics(const char *_tracking_id, bool _use_https, bool _filter_redundant_messages)
    : m_TrackingID(_tracking_id), m_ClientID(GetStoredOrNewClientID()), m_AppName(GetAppName()),
      m_AppVersion(GetAppVersion()), m_UserLanguage(UserLanguage()),
      m_Enabled(CFDefaultsGetBool(g_DefaultsTrackingEnabledKey)), m_UseHTTPS(_use_https),
      m_FilterRedundantMessages(_filter_redundant_messages)
{
    ...
    m_PayloadPrefix = "v=1"s +
                      "&"
                      "tid=" +
                      m_TrackingID + "&" + "cid=" + m_ClientID + "&" + "an=" + EscapeString(m_AppName) + "&" +
                      "av=" + m_AppVersion + "&" + "ul=" + m_UserLanguage + "&";
}
...
void GoogleAnalytics::PostScreenView(const char *_screen)
{
    ...
    std::string message = "t=screenview&cd="s + _screen;
...
void GoogleAnalytics::PostEvent(const char *_category, const char *_action, const char *_label, unsigned _value)
{
    ...
    const auto message =
        "t=event&ec="s + _category + "&ea=" + _action + "&el=" + _label + "&ev=" + std::to_string(_value);

    AcceptMessage(EscapeString(message));
}

void GoogleAnalytics::AcceptMessage(std::string _message)
{
    if( !m_Enabled )
        return;
