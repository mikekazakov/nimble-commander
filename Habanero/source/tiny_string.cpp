...
{
    __construct(s, static_cast<size_type>(strlen(s)));
}
...
void tiny_string::__construct(size_type count, char ch)
{
    if( count < __builtin_buf_size ) {
        __m_builtin.__info = __len_to_info(static_cast<unsigned>(count));
...
void tiny_string::__construct(const char *_s, size_type _count)
{
    if( _count < __builtin_buf_size ) {
        __m_builtin.__info = __len_to_info(static_cast<unsigned>(_count));
...
tiny_string::__ctrl *tiny_string::__allocate_ctrl(size_type capacity)
{
    ...
    auto c = static_cast<__ctrl *>(malloc(sz));
    if( c == nullptr )
        throw bad_alloc();
...
void tiny_string::__grow_ctrl(size_type new_capacity)
{
    ...
    if( req < min_grow )
        req = min_grow;
    auto c = static_cast<__ctrl *>(realloc(__m_ctrl, req));
    if( c == nullptr )
        throw bad_alloc();
...
tiny_string &
tiny_string::insert(size_type index, const tiny_string &str, size_type index_str, size_type count)
{
...
tiny_string &
tiny_string::replace(size_type pos, size_type count1, const const_pointer s, size_type count2)
{
    size_type sz = size();
    if( pos > sz )
        throw out_of_range("");
...
tiny_string &tiny_string::replace(size_type pos,
                                  size_type count1,
...
int tiny_string::compare(size_type pos1,
                         size_type count1,
                         const tiny_string &str,
                         size_type pos2,
...
tiny_string::size_type
tiny_string::find(const_pointer s, size_type pos, size_type count) const noexcept
{
    auto sz = size();
...
tiny_string::size_type
tiny_string::rfind(const_pointer s, size_type pos, size_type count) const noexcept
{
    auto sz = size();
    pos = std::min(pos, sz);
    if( count < sz - pos )
