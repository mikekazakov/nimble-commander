...
{
    [[clang::no_destroy]] static auto actions_map =
...
{
    [[clang::no_destroy]] static const auto ql_cache =
        std::make_shared<nc::vfsicon::QLThumbnailsCacheImpl>();
    [[clang::no_destroy]] static const auto ws_cache =
        std::make_shared<nc::vfsicon::WorkspaceIconsCacheImpl>();
    ...
    [[clang::no_destroy]] static const auto vfs_bi_cache =
        std::make_shared<nc::vfsicon::VFSBundleIconsCacheImpl>();
    ...
    [[clang::no_destroy]] static const auto icon_builder =
        std::make_shared<nc::vfsicon::IconBuilderImpl>(
...
{
    static auto decoder =
        nc::panel::ControllerStateJSONDecoder(self.nativeFSManager, self.vfsInstanceManager);
    return decoder;
}
...
- (NCPanelOpenWithMenuDelegate *)panelOpenWithMenuDelegate
{
    static const auto delegate =
        [[NCPanelOpenWithMenuDelegate alloc] initWithFileOpener:self.fileOpener utiDB:self.utiDB];
    return delegate;
}

- (nc::panel::ContextMenuProvider)makePanelContextMenuProvider
{
    auto provider = [self](std::vector<VFSListingItem> _items,
                           PanelController *_panel) -> NSMenu * {
