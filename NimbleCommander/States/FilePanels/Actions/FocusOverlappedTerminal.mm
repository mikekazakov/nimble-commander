...
bool FocusOverlappedTerminal::ValidateMenuItem(MainWindowFilePanelState *_target,
                                               NSMenuItem *_item) const
{
    if( _target.isPanelActive ) {
        _item.title = NSLocalizedString(@"Focus Overlapped Terminal",
                                        "Menu item for focusing an overlapped terminal");
    }
    else {
...
void FocusOverlappedTerminal::Perform(MainWindowFilePanelState *_target,
                                      [[maybe_unused]] id _sender) const
{
