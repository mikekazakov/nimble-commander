...
{
    // Mark all previews of PDF files as potentially hazardous and leading to a crash :-/
    auto &cmp = nc::utility::ExtensionLowercaseComparison::Instance();
    return cmp.Equal(nc::utility::PathManip::Extension(_path.native()), "pdf");
}
...
- (void)createQLView
{
    ...
    auto ql = [[NCPanelQLOverlayWrapper alloc] initWithFrame:self.frame bridge:*m_Bridge];
    ql.translatesAutoresizingMaskIntoConstraints = false;
    [self addSubview:ql];

    NSDictionary *views = NSDictionaryOfVariableBindings(ql);
    [self addConstraints:
        [NSLayoutConstraint constraintsWithVisualFormat:@"|-(==0)-[ql]-(==0)-|"
                                                options:0
                                                metrics:nil
                                                  views:views]];
    [self addConstraints:
        [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(==0)-[ql]-(==0)-|"
                                                options:0
                                                metrics:nil
                                                  views:views]];
