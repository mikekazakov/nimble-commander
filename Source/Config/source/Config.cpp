...
Token::Token(Config *_instance, unsigned long _token) noexcept : m_Instance(_instance), m_Token(_token)
{
}

Token::Token(Token &&_rhs) noexcept : m_Instance(_rhs.m_Instance), m_Token(_rhs.m_Token)
{
...
Token &Token::operator=(Token &&_rhs) noexcept
{
    if( *this )
        m_Instance->Discard(*this);
