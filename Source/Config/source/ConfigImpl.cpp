...
ConfigImpl::ConfigImpl(std::string_view _default_document,
    ...
    : m_OverwritesStorage(_storage), m_OverwritesDumpExecutor(_overwrites_dump_executor),
      m_OverwritesReloadExecutor(_overwrites_reload_executor)
{
    ...
    m_OverwritesStorage->SetExternalChangeCallback([this] { OverwritesDidChange(); });
}
...
{
    if( _value.IsInt() )
        return static_cast<T>(_value.GetInt());
    else if( _value.IsUint() )
        return static_cast<T>(_value.GetUint());
    else if( _value.IsInt64() )
        return static_cast<T>(_value.GetInt64());
    else if( _value.IsUint64() )
        return static_cast<T>(_value.GetUint64());
    else if( _value.IsDouble() )
        return static_cast<T>(_value.GetDouble());
    else
        return T{};
}
...
{
    SetInternal(_path, Value(static_cast<int64_t>(_value)));
}

void ConfigImpl::Set(std::string_view _path, unsigned long _value)
{
    SetInternal(_path, Value(static_cast<uint64_t>(_value)));
}
...
ConfigImpl::Observer::Observer(unsigned long _token, std::function<void()> _callback) noexcept
    : token{_token}, callback{std::move(_callback)}
{
}
...
static rapidjson::Document MergeDocuments(const rapidjson::Document &_main, const rapidjson::Document &_overwrites)
{
    ...
    MergeObjectsRecursively(new_document, new_document.GetAllocator(), _main, _overwrites, "");
    return new_document;
}
        ...
        if( new_it == _new.MemberEnd() ) {
            ...
            if( original_it->value.GetType() == rapidjson::Type::kObjectType )
                TraverseRecursivelyAndMarkEachMember(
                    original_it->value, _path_prefix + name.GetString() + ".", _changes);
        }
        else {
