...
+ (id)valueForKeyPath:(std::string_view)key_path inConfig:(Config &)_config
{
    ...
    switch( v.GetType() ) {
        ...
        case rapidjson::kStringType:
            return [NSString stringWithUTF8String:v.GetString()];
        case rapidjson::kNumberType:
            if( v.IsInt() )
                return [NSNumber numberWithInt:v.GetInt()];
            else if( v.IsUint() )
                return [NSNumber numberWithUnsignedInt:v.GetUint()];
            else if( v.IsInt64() )
                return [NSNumber numberWithLongLong:v.GetInt64()];
            else if( v.IsUint64() )
                return [NSNumber numberWithUnsignedLongLong:v.GetUint64()];
            else if( v.IsDouble() )
                return [NSNumber numberWithDouble:v.GetDouble()];
            else
                return nil; // future guard
        default:
            break;
    }
    return nil;
...
+ (void)setValue:(nullable id)value forKeyPath:(std::string_view)_key_path inConfig:(Config &)_config
{
    if( const auto n = nc::objc_cast<NSNumber>(value) ) {
        ...
        switch( type[0] ) {
            case 'c': // @encode(BOOL)
            case 'B': // @encode(bool)
                _config.Set(_key_path, static_cast<bool>(n.boolValue));
                break;
            case 'i': // @encode(int)
                _config.Set(_key_path, n.intValue);
                break;
            case 's': // @encode(short)
                _config.Set(_key_path, static_cast<int>(n.shortValue));
                break;
            ...
            case 'S': // @encode(unsigned short)
                _config.Set(_key_path, static_cast<unsigned int>(n.unsignedShortValue));
                break;
