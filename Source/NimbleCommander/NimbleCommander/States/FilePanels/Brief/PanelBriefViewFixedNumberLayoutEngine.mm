...
{
    if( _params.items_number < 0 || _params.item_height < 1 || _params.columns_per_screen < 1 )
    ...
    m_ClipViewWidth = static_cast<int>(_params.clip_view_bounds.size.width);
}

void FixedNumberLayoutEngine::BuildGrid(const Params &_params)
{
    const auto screen_width = static_cast<int>(_params.clip_view_bounds.size.width);
