...
ListingPromise::NonUniformListing ListingPromise::FromNonUniformListing(const VFSListing &_listing,
                                                                        const VFSPromiseAdapter &_adapter)
{
    assert(!_listing.IsUniform());

    // memory massacre 2017 AD
    using DirectoriesT = std::unordered_map<std::string_view, nc::base::StringsBulk::NonOwningBuilder>;
    ...
    for( const auto &per_vfs : info->per_vfs ) {
        ...
        for( const auto &entries : per_vfs.entries ) {
            ...
            for( size_t i = 1; i < entries.size(); ++i ) {
                ...
                const std::expected<VFSListingPtr, Error> listing =
                    host->FetchSingleItemListing(path, _fetch_flags, _cancel_checker);
                if( listing && *listing != nullptr )
...
{
    return _lhs.m_Storage == _rhs.m_Storage || *_lhs.m_Storage == *_rhs.m_Storage;
}
...
{
    return _lhs.promise == _rhs.promise && _lhs.entries == _rhs.entries;
}
