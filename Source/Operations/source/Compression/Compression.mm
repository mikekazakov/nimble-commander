...
NSString *Compression::BuildTitlePrefix() const
{
    if( m_InitialSingleItemFilename.empty() )
        return [NSString localizedStringWithFormat:NSLocalizedString(@"Compressing %d items",
                                                                     "Compressing %d items"),
                                                   m_InitialSourceItemsAmount];
    else
        return [NSString
            localizedStringWithFormat:NSLocalizedString(@"Compressing \u201c%@\u201d",
                                                        "Compressing \u201c%@\u201d"),
                                      [NSString
                                          stringWithUTF8StdString:m_InitialSingleItemFilename]];
}
