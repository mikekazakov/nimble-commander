...
PresentationItemsColoringFilter PresentationItemsColoringFilterPersitence::FromJSON(const config::Value &_v)
{
    ...
    if( _v.HasMember("executable") )
        f.executable = to_tribool(_v["executable"]);
    if( _v.HasMember("hidden") )
        f.hidden = to_tribool(_v["hidden"]);
    if( _v.HasMember("directory") )
        f.directory = to_tribool(_v["directory"]);
    if( _v.HasMember("symlink") )
        f.symlink = to_tribool(_v["symlink"]);
    if( _v.HasMember("reg") )
        f.reg = to_tribool(_v["reg"]);
    if( _v.HasMember("selected") )
        f.selected = to_tribool(_v["selected"]);
    return f;
}
