...
BriefOnDiskStorage::PlacementResult::PlacementResult(std::string _path, std::function<void()> _cleanup) noexcept
    : m_Path(std::move(_path)), m_Cleanup(std::move(_cleanup))
{
...
BriefOnDiskStorage::PlacementResult::PlacementResult(PlacementResult &&_pr) noexcept
    : m_Path(std::move(_pr.m_Path)), m_Cleanup(std::move(_pr.m_Cleanup))
{
}
