...
std::optional<TemporaryFileStorageImpl::OpenedFile> TemporaryFileStorageImpl::OpenFile(std::string_view _filename)
{
    const auto filename = _filename.empty() ? MakeRandomFilename() : std::string(_filename);
...
std::optional<std::string> TemporaryFileStorageImpl::FindSuitableExistingTempDir(std::string_view _for_filename)
{
...
std::vector<std::string> TemporaryFileStorageImpl::FindExistingTempDirectories() const
{
    ...
    while( (entry = readdir(directory)) != nullptr ) {
        if( entry->d_type != DT_DIR )
            continue;
        if( entry->d_namlen <= prefix.length() || strncmp(entry->d_name, prefix.c_str(), prefix.length()) != 0 )
            continue;
