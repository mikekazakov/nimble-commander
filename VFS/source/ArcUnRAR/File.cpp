...
File::File(const char *_relative_path, std::shared_ptr<UnRARHost> _host)
    : VFSFile(_relative_path, _host), m_UnpackBuffer(new uint8_t[m_UnpackBufferDefaultCapacity]),
...
int File::Open(unsigned long _open_flags, [[maybe_unused]] const VFSCancelChecker &_cancel_checker)
{
    ...
    while( (read_head_ret = RARReadHeaderEx(m_Archive->rar_handle, &header)) == 0 ) {
        ...
        else if( (skip_file_ret = RARProcessFile(m_Archive->rar_handle, RAR_SKIP, NULL, NULL)) !=
                 0 ) {
            break;
        }
    }
    ...
    dispatch_async(m_UnpackThread, ^{
      ...
      RARSetCallback(m_Archive->rar_handle, ProcessRAR, reinterpret_cast<long>(this));
...
{
    auto _this = reinterpret_cast<File *>(_user_data);
    ...
    switch( _msg ) {
        ...
        case UCM_PROCESSDATA: {
            ...
            if( _this->m_UnpackBufferSize + _p2 > _this->m_UnpackBufferCapacity ) {
                ...
                unsigned new_capacity = _this->m_UnpackBufferSize + static_cast<unsigned>(_p2);
                std::unique_ptr<uint8_t[]> buf(new uint8_t[new_capacity]);
                std::memcpy(buf.get(), _this->m_UnpackBuffer.get(), _this->m_UnpackBufferSize);
                _this->m_UnpackBuffer.swap(buf);
                _this->m_UnpackBufferCapacity = new_capacity;
            }

            const void *unpacked_data = reinterpret_cast<const void *>(_p1);
            std::memcpy(_this->m_UnpackBuffer.get() + _this->m_UnpackBufferSize, unpacked_data, _p2);
...
ssize_t File::Read(void *_buf, const size_t _size)
{
    ...
    assert(sz + m_Position <= static_cast<long>(m_Entry->unpacked_size));

    std::memcpy(_buf, m_UnpackBuffer.get(), sz);
    std::memmove(m_UnpackBuffer.get(), m_UnpackBuffer.get() + sz, m_UnpackBufferSize - sz);
    m_UnpackBufferSize -= sz;
    m_Position += sz;

    assert(sz <= static_cast<long>(_size));
    return sz;
}
...
bool File::Eof() const
{
    if( !m_Archive )
        return true;
    return m_Position >= static_cast<ssize_t>(m_Entry->unpacked_size);
}
