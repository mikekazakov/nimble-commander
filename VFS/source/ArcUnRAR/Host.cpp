...
public:
    ...
    const char *Tag() const { return UnRARHost::UniqueTag; }
...
UnRARHost::UnRARHost(const VFSHostPtr &_parent, const VFSConfiguration &_config)
    : Host(_config.Get<VFSArchiveUnRARHostConfiguration>().path.c_str(), _parent, UniqueTag),
      m_SeekCacheControl(dispatch_queue_create(NULL, NULL)), m_Configuration(_config)
{
...
{
    if( _archive_native_path == nullptr || _archive_native_path[0] != '/' )
        return false;
    ...
    flags.ArcName = const_cast<char *>(_archive_native_path);
...
int UnRARHost::DoInit()
{
    ...
    RAROpenArchiveDataEx flags;
    memset(&flags, 0, sizeof(flags));
    flags.ArcName = const_cast<char *>(JunctionPath());
    flags.OpenMode = RAR_OM_LIST;
    ...
    while( (read_head_ret = RARReadHeaderEx(_rar_handle, &header)) == 0 ) {
        ...
        CFStringRef utf32le =
            CFStringCreateWithBytesNoCopy(NULL,
                                          reinterpret_cast<UInt8 *>(header.FileNameW),
                                          std::wcslen(header.FileNameW) * sizeof(wchar_t),
        ...
        mode_t mode = header.FileAttr;
         // No using now. need to do some test about real POSIX mode data here, not only
        read-for-owner access.
         */
...
Directory *UnRARHost::FindOrBuildDirectory(const std::string &_path_with_tr_sl)
{
    ...
    if( find_if(begin(entries), end(entries), [&](const auto &_i) {
            return _i.name == short_name;
                                     ...
                                     [[maybe_unused]] const VFSCancelChecker &_cancel_checker)
{
    ...
    for( auto &entry : dir->entries ) {
        ...
        listing_source.sizes.insert(index,
                                    entry.isdir ? ListingInput::unknown_size : entry.unpacked_size);
...
int UnRARHost::IterateDirectoryListing(
    const char *_path,
